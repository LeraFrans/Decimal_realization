...
// // Сравнивает десятичные массивы. Возвращает: 1 если первое больше, -1 если
// // первое меньше, 0 если равны
...
//Вычитает десятичные массивы (из value_1 вычитается value_2), результат
//сохраняется в value_1

//   Нужно проверить, какое число больше, а какое меньше и добавить эту
//   обработку
// Для этого нужно написать функцию сравнения (либо сравнивать десятичне
// массивы, либо децималы) Плюс добавить обработку минусовых значений (подумать
// как передавать минусовые значения и нужно ли вообще это здесь)
...
/*
int s21_get_bit(s21_decimal value, int position) {  // -1 - ERROR, position
0-127 int res = -1; if((position > -1 && position < 128)) {
...
//Делит десятичный массив на 2. Изменяет сам массив по указателю, возвращает
//остаток от деления (либо 1, либо 0)
...
//Проверяет, состоит ли десятичный массив только из нулей (если да, то
//возвращает 1)
...
int s21_set_bit_V2(s21_decimal *result, int position,
...
// тут добавить случай, когда мантисса переполнилась и уже больше нельзя
// увеличивать её и экспоненту. Но за переполнением нужно в двоичке следить, а
// не в десятичке
...
void s21_normalization(s21_decimal value_1, s21_decimal value_2,
  ...
  int max_array[] = {5, 3, 3, 0, 5, 9, 3, 4, 5, 3, 9, 5, 7, 3, 3,
  ...
  if (difference > 0) {
    ...
    // s21_compare_ten_array(array_2, max_array) == 0) flag_owerflow_mantisa =
    // 1;
    for (int i = 0; i < difference && flag_owerflow_mantisa == 0; i++) {
      ...
      // print_ten_array(array_2);
      // printf("EXP = %d\n", exp_2);
      //делаем проверку на переполнение, сравнивая с максимальным массивом, если
      //переполнено, возвращаемся обратно на один шаг и включаем флаг
      //переполнения
      // printf("owerflow = %d\n", owerflow);
      if (s21_compare_ten_array(array_2, max_array) == 1) {
        // printf("\n\nOWERFLOW\n\n");
    ...
    //если переполнено то работаем с первым децималом, первые мантиссу и
    //экспонету уменьшаем до уровня вторых
    if (flag_owerflow_mantisa == 1) {
      ...
      //просто скопировать первый децимал в результирующий, если мы его не
      //трогали
    ...
    // s21_compare_ten_array(array_1, max_array) == 0) flag_owerflow_mantisa =
    // 1;
    for (int i = 0; i < difference && flag_owerflow_mantisa == 0; i++) {
      exp_1++;
      //умножаем на 10 один раз
    ...
    // print_ten_array(array_1);
    s21_from_ten_array_to_decimal(array_1, result_1);
    //если переполнено то работаем со вторым децималом, вторые мантиссу и
    //экспонету уменьшаем до уровня вторых
    if (flag_owerflow_mantisa == 1) {
      int array_2[30] = {0};
      s21_from_decimal_to_ten_array(value_2, array_2);
      ...
      //просто скопировать второй децимал в результирующий, если мы его не
      //трогали printf("\n\n\nJJJJJJJJ\n\n");
...
//нормлизацию, а через мою функцию сложения, просто сдвинуть индекс (вычитание
//так же)
...
// printf("Array final: ");
// for (int i = 0; i<30; i++) printf("%d", a[i]);
//    s21_decimal first = {12345, 0, 4294967,
//    0b10000000000000101000000000000000}; printf("exp 1 = %d\n",
//    s21_get_exp(first)); s21_decimal second = {4294967293, 4294967295,
//    4294967, 0b00000000000000001000000000000000};
